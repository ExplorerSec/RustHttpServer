...
use core::str;
use std::{collections::HashMap, fs, io::prelude::*, net::TcpStream, sync::{Arc, Mutex}, thread, time::Duration};
...
pub fn handle_stream(mut stream: TcpStream, auth: Arc<Mutex<Auth>>) {
        ...
        if let Some((method_path, _)) = start_line.rsplit_once(' ') {
            let (status, head, body) = match method_path {
                ...
                "POST /login" => {
                    ...
                    if let Some(cookie) = auth.login(content){
                        (
                            HttpStatus::Status200,
                ...
                "GET /user" => {
                    ...
                    if let Some(cookie) = msg.head.get("Cookie") {
                        let auth = auth.lock().unwrap();
                        if auth.judge_login(cookie){
                            resp = (
                                HttpStatus::Status200,
                                "",
                ...
                "CONNECT cn.bing.com:443" => (HttpStatus::Status200, "", HttpBody::Text("Hello")),
